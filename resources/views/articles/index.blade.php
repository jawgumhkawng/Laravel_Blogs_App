...
<button type="button" class="  mt-2 float-end link text-primary " title="View Profile" style="border:none; cursor:pointer; padding:0; background:none; font-size:20px; " data-bs-toggle="modal" data-bs-target="#exampleModal" data-bs-whatever="@getbootstrap"> <i class="fa-solid fa-user"></i></button>
 
</div>

<div class="modal fade " id="exampleModal" style="user-select: none" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="card d-flex justify-content-center align-items-center" style="width: 100%;">
        @if($user->image)

           <img src='{{ url("./images/$user->image") }}' class="card-img-top rounded-circle mt-4" style="border: 2px solid gray; height:220px; width:220px"  alt="..."><br> 
...
<hr>
        ...
        <div class="d-flex mt-3">
            ...
            <img src="{{ url('./images/'.$article->image) }}" class="card-img-top " style=" width: 100%;"  alt="...">
                <div class="card-body">
                    ...
                    <b>Author : </b><b class="text-success">{{ $article->user->name}}</b><br>
                     ...
                     <b>Category : </b><span class="btn btn-sm btn-success p-0 px-1 m-0 text-white" > {{ $article->category->name }}</span><br>
                    @elseif($article->category->id == 2) 
                    <b>Category : </b><span class="btn btn-sm btn-info p-0 px-1 m-0 text-white" > {{ $article->category->name }}</span><br>
                    @elseif($article->category->id == 3)
                    <b>Category : </b><span class="btn btn-sm btn-secondary p-0 px-1 m-0 text-white" > {{ $article->category->name }}</span><br>
                    @elseif($article->category->id == 4)
                    <b>Category : </b><span class="btn btn-sm btn-primary p-0 px-1 m-0 text-white" > {{ $article->category->name }}</span><br>
                    @else
                    <b>Category : </b><span class="btn btn-sm btn-warning p-0 px-1 m-0 text-white" > {{ $article->category->name }}</span><br>
                    @endif
                    <b >Comments : </b> <b class="text-primary" > {{ count( $article->comments) }}</b> | 
